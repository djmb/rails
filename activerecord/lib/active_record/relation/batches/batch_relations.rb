    ...
    class BatchRelations
      ...
      private
        ...
        def offset_from_single_column(ordering, offset)
          ordering.ascending? ? ordering.expr.gt(offset) : ordering.expr.lt(offset)
        end

        def offset_from_multiple_columns
          OffsetFromMany.new(orderings, offsets, primary_key_column, connection.sorts_nulls_first?).clause
        end

        class OffsetFromMany
          attr_reader :order_offsets, :primary_key_column, :arel_table

          def initialize(orderings, offsets, primary_key_column, sorts_nulls_first)
            @order_offsets = orderings.zip(offsets)
            @primary_key_column = primary_key_column
            @sorts_nulls_first = sorts_nulls_first
            @arel_table = orderings.first.expr.relation
          end
          ...
          private
            ...
            def subsequent_to_value(ordering, value)
              ...
              next_values = ordering.ascending? ? column.gt(value) : column.lt(value)
