    ...
    private
      def process_batches_order_arg(order)
        ...
        if VALID_DIRECTIONS.include?(order)
          ...
          case order
          when Symbol
            orderings << process_batches_symbol_order_arg(order)
          when Hash
            orderings.concat(process_batches_hash_order_arg(order))
          when Array
            order.each do |arg|
              case arg
              when Symbol
                orderings << process_batches_symbol_order_arg(arg)
              when Hash
                orderings.concat(process_batches_hash_order_arg(arg))
              else
                raise ArgumentError, "Order argument \"#{arg}\" is invalid. Must be Symbol or Hash"
              end
            end
          else
            raise ArgumentError, "Order \"#{order}\" is invalid. Must be Symbol, Hash or Array"
          end

          # Add the primary key to the ordering to ensure rows have unique sort keys
          unless orderings.any? { |ordering| ordering.expr == primary_key_column }
            orderings << primary_key_column.asc
          end
          orderings
        end
      end

      def process_batches_symbol_order_arg(arg)
        order_column(arg.to_s).asc
      end

      def process_batches_hash_order_arg(arg)
        arg.map do |field, dir|
          unless VALID_DIRECTIONS.include?(dir)
            raise ArgumentError, "Direction \"#{dir}\" is invalid. Valid directions are: #{VALID_DIRECTIONS.to_a.inspect}"
          end
